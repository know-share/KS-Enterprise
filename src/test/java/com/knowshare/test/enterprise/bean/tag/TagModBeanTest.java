...
public class TagModBeanTest extends AbstractTest{
	...
	public void test01Create(){
		boolean result = tagModBean.create("NEW TAG");
		assertTrue(result);
	...
	public void test02Update(){
		...
		boolean result = tagModBean.update(tag);
		assertTrue(result);
		tags = mongoTemplate
		...
		result = tagModBean.update(new Tag());
		assertFalse(result);
	}
	...
	public void test03Delete(){
		List<Tag> tags = mongoTemplate
				.find(new Query(Criteria.where("nombre").is("TAG UPDATED")), Tag.class);
		boolean result = tagModBean.delete(tags.get(0).getId());
		assertTrue(result);
		tags = mongoTemplate
				.find(new Query(Criteria.where("nombre").is("TAG UPDATED")), Tag.class);
		assertEquals(0, tags.size());
		
		result = tagModBean.delete("");
		assertFalse(result);
	}
